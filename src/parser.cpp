...
namespace qarser {
    ...
    void Parser::error(const std::string& message) {
        throw ParseError(current.line, current.column, message);
    }
    ...
    std::unique_ptr<Statement> Parser::parse_statement() {
        ...
        if (match(TokenType::GATE)) return parse_gate();
    ...
    std::unique_ptr<CRegister> Parser::parse_creg() {
        ...
        auto [name, size] = parse_array_declaration();
        return std::make_unique<CRegister>(CRegister{name, size});
    }
    ...
    std::unique_ptr<Measure> Parser::parse_measure() {
        ...
        Token arrow = consume(TokenType::ARROW, "Need '->'!");
